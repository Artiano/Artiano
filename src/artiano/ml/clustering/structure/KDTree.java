...
/**
 * <p>
 * Description: KD Tree for KMeans.
 * </p>
 * 
 * @author JohnF Nash
 * @version 1.0.0
...
public class KDTree extends BaseKDTree {
	...
	private BaseKDNode buildKDTree(Matrix dataSet) {
		...
		BaseKDTree.BaseKDNode root = new BaseKDTree.BaseKDNode(dataSet); // 初始化根节点
	...
	private void partition_features(BaseKDTree.BaseKDNode kdNode) {
		...
		int partitionFeatureIndex = getPartitionFeatureIndex(kdNode.treeData); // Get
																				// partition
																				// feature
																				// index
		...
		kdNode.partitionValue = data.at(data.rows() / 2, partitionFeatureIndex); // Partition
																					// key
																					// value.
	 ...
	 *            - 分裂属性在属性列表中的下标
	 */
	private void sortFeatureByIndex(BaseKDTree.BaseKDNode node,
