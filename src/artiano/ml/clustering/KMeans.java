...
public class KMeans extends Clustering {
	...
	private Map<Matrix, Matrix> findFinalCenters(Matrix data, int k) {
		// 获取随机产生中心点的次数
		final int NUMOFGENERATINGCENTERS = computeNumOfCombinations(
				data.rows(), k); // / 7;
		// 存储NUMOFGENERATINGCENTERS次实验的所有的最终中心点
		Matrix allExerCenters = new Matrix(NUMOFGENERATINGCENTERS * k,
				data.columns());
		// 存储所有的聚类结果
		...
		for (int i = 0; i < NUMOFGENERATINGCENTERS; i++) {
			...
			allExerCenters.set(new Range(i * k, i * k + k),
		...
		minEvaluation = allEvaluations.at(0, 0);
		int minIndex = 0;
	...
	/**
	 * k-means Find output matrix of the cluster centers, one row per cluster
	 * center
	 * 
	 * @param centers
	 *            簇中心
	 * @return 聚类得到的所有簇，簇中心点作为键，簇内数据点作为值 Output matrix of the cluster centers,
	...
	private Matrix getUpdatedCenters(Map<Matrix, Matrix> clusterMap, int columns) {
		...
		for (Entry<Matrix, Matrix> entry : entrySet) {
			...
			for (int j = 0; j < columns; j++) {
				...
				double value = Double.parseDouble(f.format(sum
						/ aCluster.rows())); // 实数格式化
	...
	private Matrix getNonCenterData(Matrix centers) {
		...
		// 获取数据集中的非中心点
		Matrix remaining = new Matrix(data.rows() - centerInDataNum,
				data.columns());
		int count = 0;
	...
	private double distance(Matrix mat1, Matrix mat2) {
		if (mat1.columns() != mat2.columns() || mat1.rows() != 1
				|| mat2.rows() != 1) {
	...
	private int computeNumOfCombinations(int n, int r) {
		/**
		 * int result = 1; for (int i = 0; i < r; i++) { result *= (n - i); }
		 * for (int i = r; i > 0; i--) { result /= i; } return result;
		 */
		return this.k;
	}
	...
	public class KMeansCluster extends ClusterModel {

		public KMeansCluster(Table data) {
			super(data);
		}

		private Matrix mGlobalMean;
		private HashMap<Object, Matrix> mCentroids = new HashMap<>();
		private double mEvaluation;
		private Matrix mNoise;
		 ...
		 * @param centroid
		 */
		public void assignCentroidToCluster(Object cluster, Matrix centroid) {
			mCentroids.put(cluster, centroid);
		}
		 ...
		 * @return
		 */
		public Matrix getCentroidAssignedToCluster(Object cluster) {
			return mCentroids.get(cluster);
		}
		
		public void setGlobalMean(Matrix global) {
			mGlobalMean = global;
		}
		...
		public void setNoise(Matrix matrix) {
			mNoise = matrix;
		}
		...
		public String toString() {
			...
			// evaluation
			builder.append("<pre>最小和方误差："+mEvaluation+"</pre>");
			// centroid
			ArrayList<Object> keys = new ArrayList<>(mCentroids.keySet());
			builder.append("<br>");
			builder.append("<table border='1' cellspacing='0px'>");
			builder.append("<caption>聚类中心</caption>");
			builder.append("<tr><td>centroid</td>");
			for (int i = 0; i < super.mDataSet.columns(); i++) {
				builder.append("<td>"+mDataSet.attribute(i).getName()+"</td>");
			}
			builder.append("</tr>");
			builder.append("<tr><td>global</td>");
			for (int i = 0; i < mGlobalMean.size(); i++) {
				builder.append("<td>"+format.format(mGlobalMean.at(i))+"</td>");
			}
			builder.append("</tr>");
			for (int i = 0; i < keys.size(); i++) {
				builder.append("<tr>");
				builder.append("<td>"+keys.get(i)+"</td>");
				Matrix mat = mCentroids.get(keys.get(i));
				for (int j = 0; j < mat.size(); j++)
					builder.append("<td>"+format.format(mat.at(j))+"</td>");
				builder.append("</tr>");
			}
			builder.append("</table>");
			if (mNoise.rows() > 1) {
				builder.append("<br><p>噪声点</p>");
				...
				builder.append("</tr>");
				for (int i = 1; i < mNoise.rows(); i++) {
					builder.append("<tr>");
					for (int j = 0; j < mNoise.columns(); j++) {
						builder.append("<td>"+format.format(mNoise.at(i, j))+"</td>");
					}
					builder.append("</tr>");
				}
				builder.append("</table>");
	...
	public ClusterModel cluster(Table data) {
		Matrix mat = data.toMatrix();
		Map<Matrix, Matrix> clusters = kmeans(mat, k);
		// construct cluster model
		KMeansCluster cluster = new KMeansCluster(data);
		cluster.setGlobalMean(mat.rowMean());
		Set<Entry<Matrix, Matrix>> entrySet = clusters.entrySet();
		int count = 0;
		// assigned cluster to instance
		for (Entry<Matrix, Matrix> entry : entrySet) {
			String label = "#" + count;
			cluster.assignCentroidToCluster(label, entry.getKey());
			count++;
			Matrix points = entry.getValue();
			for (int i = 0; i < points.rows(); i++) {
				int idx = mat.indexOf(points.row(i));
				if (idx != -1)
					cluster.assignClusterToInstance(idx, label);
			}
		}
		cluster.setEvaluation(minEvaluation);
		// noise
		Matrix noise = new Matrix(1, mat.columns());
		for (int i = 0; i < mat.rows(); i++) {
			if (cluster.getClusterAssignedToInstance(i) == null) {
				noise.mergeAfterRow(mat.row(i));
			}
		}
		cluster.setNoise(noise);
		return cluster;
	}
