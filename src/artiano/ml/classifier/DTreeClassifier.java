...
public class DTreeClassifier extends Classifier {
	...
	public NominalAttribute predict(Table dataSet) {
		...
		for (int i = 0; i < dataSet.rows(); i++) {
			...
			while (current.nextNodes.size() >= 1) {
				...
				ArrayList<ArrayList<Object>> attributeValueList = constructAttributeValueList(
						this.trainset, this.attributeList);
				if (!attributeValueList.get(indexOfAttr)
						.contains(valueSearched)) {
					break;
				}
	...
	private void constructSubTree(DTreeNode p, Table remainingTrainSet,
		...
		ArrayList<ArrayList<Object>> attributeValueList = constructAttributeValueList(
				remainingTrainSet, remainingAttribute);
		...
		for (int j = 0; j < attrValues.size(); j++) {
			Table newRemainingData = getNewRemainingData(remainingTrainSet,
			...
			if (newRemainingData.rows() == 0) { // 这个分支已经没有节点
				new_node.label = mostCommonLabel(remainingTrainSet
						.classAttribute());
				if (p.nextNodes == null) {
	...
	// 更新还未得到匹配的数据集
	private Table getNewRemainingData(Table remainingTrainSet, int indexOfAttr,
			Object attrValue) {
		...
		while (attrIter.hasNext()) {
			...
			if (attr instanceof NumericAttribute) {
				newRemainingData.addAttribute(new NumericAttribute(attr
						.getName(), new IncrementVector()));
			} else if (attr instanceof NominalAttribute) {
				newRemainingData.addAttribute(new NominalAttribute(attr
						.getName(), new IncrementVector()));
			}
		}
		// 指定类标属性
	...
	private int getMaxInformationGainAttribute(Table remainingTrainSet,
			ArrayList<String> remainingAttribute) {
		double max_gain = 0;
		int max_index = 0; // 拥有最大信息增益的属性的下标
		for (int i = 0; i < remainingAttribute.size(); i++) {
			// 获取当前属性的信息增益
			double temp_gain = computeInformationGain(remainingTrainSet,
					remainingAttribute.get(i));
	...
	private double computeInformationGain(Table remainingTrainSet,
		...
		double inforGain = 0;
		// Add entropy(S);
		ArrayList<Integer> labelCounts = countAttributeValuesApperances(remainingTrainSet
				.classAttribute());
		...
		ArrayList<Integer> eachCount = countAttrValuesApperances(
				remainingTrainSet, indexOfAttr);
		...
		for (int j = 0; j < eachCount.size(); j++) {
			double entropy = getEntropy(remainingTrainSet, indexOfAttr,
					attrValues.get(j));
			inforGain += (-1.0 * eachCount.get(j)) / remainingTrainSet.rows()
					* entropy;
		}
		return inforGain;
	}
	 ...
	 *            - 还未被分类的数据集
	 * @param attrIndex
	 *            - 当前需要计算属性值的信息增益的属性在属性列表中的下标
	 * @param attrValue
	 *            - 当前使用的属性值
	 * @return 该属性值的信息增益
	 */
	private double getEntropy(Table remainingTrainSet, int attrIndex,
			Object attrValue) {
		ArrayList<Integer> labelValueCounts = countLabelsForSpecifiedAttrValue(
				remainingTrainSet, attrIndex, attrValue);
	...
	private ArrayList<Integer> countAttrValuesApperances(
		...
		Map<Object, Integer> attrValueCountsMap = new LinkedHashMap<Object, Integer>();
		for (int i = 0; i < remainingTrainSet.rows(); i++) {
	...
	private ArrayList<Integer> countLabelsForSpecifiedAttrValue(Table trainset,
			int indexOfAttr, Object attrValue) {
		...
		for (int i = 0; i < trainset.rows(); i++) {
			...
			if (!attrValueCountsMap.containsKey(label)) {
				...
				attrValueCountsMap
						.put(label, attrValueCountsMap.get(label) + 1);
			}
		}
		ArrayList<Integer> attrValueCounts = new ArrayList<Integer>(
				attrValueCountsMap.values());
	...
	private ArrayList<ArrayList<Object>> constructAttributeValueList(
		...
		ArrayList<ArrayList<Object>> attributeValueList = new ArrayList<ArrayList<Object>>();
