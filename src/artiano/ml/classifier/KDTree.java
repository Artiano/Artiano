...
public class KDTree extends BaseKDTree {
	...
	public KDTree(Table trainSet) {
		// 获取训练集去掉类标那一列之后剩下的数据
		Matrix dataSetMat = generateTrainDataWithoutLabel(trainSet);
	...
	public boolean delete(BaseKDNode nodeToDelete) {
		...
		Matrix newTreeData = new Matrix(root.treeData.rows() - 1,
				root.treeData.columns());
	...
	public KDNode findNearest(KDNode root, Matrix target) {
		...
		// 节点数据
		KDNode nearestNode = searchPath.peek();
		...
		while (!searchPath.empty()) {
			...
			// 作为数据分类的属性
			featureIndex = back_point.featureIndex; 
			double dist1 = distance(target.column(featureIndex),
	...
	private void partition_features(KDNode kdNode) {
		...
		int partitionFeatureIndex = getPartitionFeatureIndex(kdNode.treeData);
		...
		// Partition key value.
		kdNode.partitionValue = data.at(data.rows() / 2, partitionFeatureIndex);
	...
	private void sortFeatureByIndex(KDNode node, final int featureIndex) {
		...
		for (int k = 1; k < dataList.size() && needNextPass; k++) {
			needNextPass = false; // Array may be sorted and next pass not
									// needed
		...
		Matrix sortedTreeData = new Matrix(dataSet.rows(), dataSet.columns(),
				data);
