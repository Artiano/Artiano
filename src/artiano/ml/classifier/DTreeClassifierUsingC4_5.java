...
public class DTreeClassifierUsingC4_5 extends Classifier {
	...
	public boolean train(Table trainSet) {
		...
		if (!capability.handles(trainSet))
			return false;
		intitialize(trainSet);
		// 深度复制trainLabel
		NominalAttribute copyOfTrainLabel = deepCloneTrainLabel((NominalAttribute) trainSet
				.classAttribute());
		// 构造决策树
	...
	private NominalAttribute deepCloneTrainLabel(NominalAttribute trainLabel) {
		NominalAttribute copyOfTrainLabel = new NominalAttribute(
				trainLabel.getName(), trainLabel.getVector().copy());
	...
	public NominalAttribute predict(Table data) {
		...
		NominalAttribute predictionAttr = new NominalAttribute("label"); // Store
																			// predictions
		for (int i = 0; i < data.rows(); i++) {
			...
			while (current.nextNodes.size() >= 1) {
				...
				ArrayList<ArrayList<String>> attributeValueList = constructAttributeValueList(
				...
				if (!isAttributeContinuous[attrIndexInOrigin]) {
					...
					// Search each branch of an decision variable to match the sample
					List<DTreeNode> childrenNodes = current.nextNodes;
					double middleValue = Double.parseDouble(childrenNodes
							.get(0).previousDecision);
			 ...
			 predictionAttr.push(current.label); } else { int attributeIndex =
			 this.attributeList.indexOf(current.attribute); if(!searchComplete
			 && !isAttributeContinuous[attributeIndex]) {
	...
	private DTreeNode constructDecisionTree(DTreeNode p,
		...
		// All the attributes has been considered ,yet not complete the
		// classification
		if (remainingAttribute.size() == 0 && remainingData.size() > 0) {
			final int indexOfAttr = attributeList.indexOf(p.attribute);
			System.out.println("attr: " + p.attribute + ", index: "
					+ indexOfAttr);
			if (indexOfAttr < 0) {
				return p;
			}
		...
		int max_index = getMaxGainRatioAttribute(remainingData,
	...
	private void constructContinuousSubTree(DTreeNode p,
			...
			ArrayList<String> remainingAttribute,
			NominalAttribute remainingTrainLabel) {
		ArrayList<ArrayList<String>> copyOfData = new ArrayList<ArrayList<String>>(
				remainingData);
		...
		int maxInfoGainIndex = getMaxAttributeInfoGainIndex(copyOfData,
				remainingTrainLabel);
		if (remainingData.size() == 0) {
			p.label = mostCommonLabel(remainingTrainLabel);
		}

		ArrayList<ArrayList<String>> leftChildData = new ArrayList<ArrayList<String>>();
		NominalAttribute leftChildTrainLabel = new NominalAttribute(
				remainingTrainLabel.getName(), new IncrementVector());
		...
		ArrayList<ArrayList<String>> rightChildData = new ArrayList<ArrayList<String>>();
		NominalAttribute rightChildTrainLabel = new NominalAttribute(
				remainingTrainLabel.getName(), new IncrementVector());
		...
		double leftData = Double.parseDouble(copyOfData.get(maxInfoGainIndex)
				.get(indexOfAttr));
		double rightData = Double.parseDouble(copyOfData.get(
				maxInfoGainIndex + 1).get(indexOfAttr));
		double middle = (leftData + rightData) / 2;
		System.out.println(p.attribute + ", left:  " + leftData + ", right: "
				+ rightData);
	...
	private void constructDiscreteAttributeSubTree(DTreeNode p,
		...
		ArrayList<ArrayList<String>> attributeValueList = constructAttributeValueList(
				remainingData, remainingAttribute);
		...
		for (int j = 0; j < attrValues.size() && j < 4; j++) {
			ArrayList<ArrayList<String>> newRemainingData = getNewRemainingData(
					remainingData, attrIndexInOrigin, attrValues.get(j));
			NominalAttribute newRemainingTrainLabel = getNewRemainingTrainLabel(
					newRemainingData, attrIndexInOrigin, attrValues.get(j),
					remainingTrainLabel);

			DTreeNode new_node = new DTreeNode(); // Root of the sub tree
			new_node.previousDecision = attrValues.get(j);
			if (newRemainingData.size() == 0) { // Now has no sample of this
												// branch
				new_node.label = mostCommonLabel(remainingTrainLabel);
				if (p.nextNodes == null) {
					p.nextNodes = new ArrayList<DTreeNode>();
				}
				p.nextNodes.add(new_node); // Add root of the sub tree to the
											// node
				break;
			} else {
	...
	private int getMaxAttributeInfoGainIndex(
		...
		for (int i = 0; i < remainingData.size() - 1; i++) {
			...
			for (int j = 0; j <= i; j++) {
				int labelValueIndex = nominals.indexOf(remainingTrainLabel
						.get(i));
			...
			for (int j = 0; j < nominals.size(); j++) {
				...
				leftInfoGain += -1 * refactor * Math.log(refactor)
						/ Math.log(2);
			}
			...
			for (int j = 0; j < nominals.size(); j++) {
				...
				double refactor = ((double) rightEachLabelCount[j])
						/ rightTotalCount;
				rightInfoGain += -1 * refactor * Math.log(refactor)
						/ Math.log(2);
			}

			double infoGain = ((double) (i + 1)) / remainingData.size()
					* leftInfoGain + ((double) rightTotalCount)
					/ remainingData.size() * rightInfoGain;
	...
	private ArrayList<ArrayList<String>> getNewRemainingData(
			...
			String attrValue) {
		ArrayList<ArrayList<String>> newRemainingData = new ArrayList<ArrayList<String>>();
	...
	private NominalAttribute getNewRemainingTrainLabel(
		...
		NominalAttribute newRemainingTrainLabel = new NominalAttribute(
				remainingTrainLabel.getName(), new IncrementVector());
		for (int i = 0; i < remainingData.size(); i++) {
			ArrayList<String> currentItem = remainingData.get(i);
			if (attrValue.equals(currentItem.get(indexOfAttr))) {
	...
	private int getMaxGainRatioAttribute(
			ArrayList<ArrayList<String>> remainingData,
			ArrayList<String> remainingAttribute,
			NominalAttribute remainingTrainLabel) {
		double max_gain = 0;
		int max_index = 0; // Attribute index where the attribute information
							// gain max
		for (int i = 0; i < remainingAttribute.size(); i++) {
			// Get information gain of the attribute
			double temp_gain;
			if (isAttributeContinuous[i]) {
				int attrIndex = this.attributeList.indexOf(remainingAttribute
						.get(i));
				temp_gain = getGainRatioForContinuousAttribute(remainingData,
						remainingTrainLabel, attrIndex);
			} else {
				temp_gain = computeGainRatio(remainingData,
						remainingAttribute.get(i), remainingTrainLabel);
			}
	 ...
	 * 
	 * @param remainingData
	 *            - remaining data to be classified.
	 * @param attribute
	 *            - attribute to compute information gain. ***Attention: @param
	 *            attrIndex - index of attribute to compute information gain.
	...
	private double computeSplitInformation(
		...
		ArrayList<Integer> eachCount = countAttributeValuesApperances(
				remainingData, indexOfAttr);

		/**
		 * Get remaining values of attribute in indexOfAttr Can not use
		 * ArrayList<String> attrValues =attributeValueList.get(indexOfAttr)
		 */
	...
	private double computeInformationGain(
		...
		for (int j = 0; j < attrValues.size(); j++) {
			double entropy = getEntropy(remainingData, indexOfAttr,
					attrValues.get(j), remainingTrainLabel);
			inforGain += (-1.0 * eachCount.get(j)) / remainingData.size()
					* entropy;
		}
		return inforGain;
	}
	 ...
	 *            - remaining data to be classified.
	 * @param attrIndex
	 *            - index of the attribute to compute entropy.
	 * @param attrValue
	...
	private double getEntropy(ArrayList<ArrayList<String>> remainingData,
			...
			NominalAttribute remainingTrainLabel) {
		Map<Object, Integer> countMap = countEachLabelValue(remainingData,
				attrIndex, attrValue, remainingTrainLabel);
		List<Integer> countList = new ArrayList<Integer>(countMap.values());
	...
	private double getGainRatioForContinuousAttribute(
			ArrayList<ArrayList<String>> remainingData,
			NominalAttribute remainingTrainLabel, final int attrIndex) {
		ArrayList<ArrayList<String>> copyOfData = new ArrayList<ArrayList<String>>(
				remainingData);
		Collections.sort(copyOfData, new Comparator<ArrayList<String>>() {
			@Override
		...
		double splitInfo = 0;
		double refactor_1 = ((double) (maxInfoGainIndex + 1))
				/ remainingData.size();
		splitInfo += -1 * refactor_1 * Math.log(refactor_1) / Math.log(2.0);
		double refactor_2 = ((double) (remainingData.size() - maxInfoGainIndex - 1))
				/ remainingData.size();
	...
	private Map<Object, Integer> countEachLabelValue(
			ArrayList<ArrayList<String>> remainingData, int attrIndex,
			String attrValue, NominalAttribute remainingTrainLabel) {
		Map<Object, Integer> labelMap = new LinkedHashMap<Object, Integer>();
		for (int i = 0; i < remainingData.size(); i++) {
	...
	private ArrayList<Integer> countAttributeValuesApperances(
			ArrayList<ArrayList<String>> remainingData, int indexOfAttr) {
		Map<String, Integer> attrValueCountsMap = new LinkedHashMap<String, Integer>();
		for (int i = 0; i < remainingData.size(); i++) {
			...
			if (!attrValueCountsMap.containsKey(attrValue)) {
				...
				attrValueCountsMap.put(attrValue,
						attrValueCountsMap.get(attrValue) + 1);
			}
		}
		ArrayList<Integer> attrValueCounts = new ArrayList<Integer>(
				attrValueCountsMap.values());
	...
	private ArrayList<ArrayList<String>> constructAttributeValueList(
			ArrayList<ArrayList<String>> data, ArrayList<String> attributeList) {
		...
		for (int i = 0; i < data.size(); i++) {
			...
			for (int j = 0; j < attributeList.size(); j++) {
				int attrIndex = this.attributeList
						.indexOf(attributeList.get(j));
				// Attention:
