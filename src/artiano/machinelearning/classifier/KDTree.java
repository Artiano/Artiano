...
public class KDTree {
	...
	public KDNode buildKDTree(Matrix dataSet) {
		...
		root = new KDNode(dataSet); //Initialize the root node		
	...
	public boolean delete(Matrix data) {
		...
		while(current != null) {
			...
			if(current != null) {  //Next feature comparision
				featureIndex = current.featureIndex;
			}
		}
		...
		if(current.left == null) {
			...
			current.nodeData = rightMost.nodeData;
			current.partitionValue = rightMost.partitionValue;
	 ...
	 */
	public Matrix findNearest(Matrix target) {
		if(root == null) {
			return null;
		}			
