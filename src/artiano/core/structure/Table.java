...
public class Table {
	...
	/**
	 * 将表按列分割为多个表
	 * @param indicesToSplit 表的列索引
	 * @return
	 */
	public Table[] split(int[][] indicesToSplit) {
		// split
		Table[] t = new Table[indicesToSplit.length];
		for (int i = 0; i < t.length; i++) {
			t[i] = new Table();
			t[i].rows = rows;
			t[i].index = this.index;
			for (int j=0; j<indicesToSplit[i].length; j++)
				t[i].addAttribute(attribute(indicesToSplit[i][j]));
		}
		return t;
	}
	...
	public Object[][] to2DArray() {
		Object[][] x = new Object[rows()][columns()];
		for (int i = 0; i < rows(); i++)
	...
	public class TableRow {
		...
		public boolean equals(Object x) {
			if (!(x instanceof TableRow))
				return false;
			TableRow tableRow = (TableRow) x;
			for (int i = 0; i < tableRow.row.length; i++)
				if (!tableRow.row[i].equals(row[i]))
					return false;
			return true;
		}
